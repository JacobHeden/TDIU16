...
syscall_handler (struct intr_frame *f) 
{
  ...
  switch (*esp)
    ...
    case SYS_EXIT : ;
      ...
      //Kollar ESP[1] är vaild :) Kanske inte behövs eftesom vi gör en process exit men vi sätter -1 då :))) lite onödig kan tyckas. 
    ...
    case SYS_WRITE :
      ...
      if(is_kernel_vaddr (esp[1]) || is_kernel_vaddr (esp[3]) || !verify_fix_length(esp[2], esp[3]))
      	{
	  process_exit(-1);
	  thread_exit();
       ...
       if( (char*)esp[1] == NULL || is_kernel_vaddr (esp[1])|| !verify_variable_length(esp[1]))
	{
      ...
      break;
    case SYS_CREATE :
      //sys create esp2 är size på filen inte på namnet!! 
    ...
    case SYS_CLOSE :
      { 
	//Behöver man stänga ner tråden ?!?! Nej !1!! inget som kan förstöra något trevligt i pintos.- 
	if(!is_kernel_vaddr (esp[1]))
	  {
	    s_close(esp);
	  }
    ...
    case SYS_SEEK :
      {
	/*	if(is_kernel_vaddr(esp[1]) || is_kernel_vaddr(esp[2]))  Inte kritisk 
	  {
	    process_exit(-1);
	    thread_exit();
	    }*/
    ...
    case SYS_TELL :
      {
	/*if(is_kernel_vaddr (esp[1])) inte kritiskt
	  {
	    process_exit(-1);
	    thread_exit();
    ...
    case SYS_FILESIZE :
      {
	/*
	if(is_kernel_vaddr (esp[1])) inte kritisk 
	  {
	    process_exit(-1);
	    thread_exit();
